function [acc,label_cv,W]=LeaveOneSubjectOutCV_Coherence(idx,dim,Coh,Label,iter,order)
    % beta:9  gamma:9
    subjects=32;
    sessions=1;
    trials=40;
    ...
    for f=1:F
        data_test=squeeze(Coh(f,:,:,idx_test));
        data_train=squeeze(Coh(f,:,:,idx_train));
        
        W(:,:,f)=DR_Coherence(data_train,label_train,dim,iter);
