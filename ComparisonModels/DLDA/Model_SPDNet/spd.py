...
from DLDA.Model_SPDNet.utils import *
from DLDA.Model_SPDNet import StiefelParameter
...
class MySPDNet(nn.Module):
    def __init__(self, n_class, input_size, hidden_size, lda_args):
        super(MySPDNet, self).__init__()
        self.lda_args = lda_args
        self.unit1 = hidden_size
        ...
        self.dropout = nn.Dropout()
        if self.lda_args:
            self.lda = LDA(n_class, lda_args['lamb'])
        else:
            self.linear = nn.Linear(self.unit_tangent, n_class)
    ...
    def forward(self, x, y, training):
        ...
        x = self.dropout(x)
        if self.lda_args:
            hasComplexEVal, out = self.lda(x, y, training)  # evals
            return hasComplexEVal, x, out
        else:
            # out = self.linear(x)
            return x
