...
from Model.ODERGRU import ODERGRU
from ODERGRU.train_ODERGRU import train_op, test_op
from Model.utils import BaseDataset, calculate_metrics
...
if __name__ == "__main__":
    ...
    parser.add_argument('--latents', default=62, type=int, help='Latents dimension')
    parser.add_argument('--units', default=31, type=int)
    ...
    for sub in range(1, args.subjects + 1):
        ...
        # Dataset
        dataset = BaseDataset(X1, Y1, args)
        ...
        for fold, (train_ids, test_ids) in enumerate(kfold.split(dataset)):
            ...
            if args.model == "ODERGRU":
                model = ODERGRU(n_class=args.classes, n_layers=1, n_units=100, latents=args.latents, units=args.units, T=args.T, channel=args.channel, bi=args.bi, device=device).to(device)
