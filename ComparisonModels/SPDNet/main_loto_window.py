...
from SPDNet.Model_SPDNet.spd import MySPDNet
from SPDNet.Model_SPDNet.optimizer import StiefelMetaOptimizer
from SPDNet.utils import BaseDataset_window_loto, BaseDataset_static, calculate_metrics
...
if __name__ == "__main__":
    ...
    for latent in dim:
        ...
        for sub in range(1, args.subjects + 1):
            ...
            Y = Data['Label'].squeeze().astype('float32')
            twoclass_indices = np.where((Y == -1) | (Y == 1))
            X = X[twoclass_indices]
            Y = Y[Y != 0]
            Y[Y == -1] = 0 

            dataset = BaseDataset_window_loto(X, Y)
            ...
            for fold, (train_ids, test_ids) in enumerate(kfold.split(dataset)):
                ...
                trDL = DataLoader(dataset, batch_size=args.batch, sampler=train_subsampler, drop_last=True)
                valDL = DataLoader(dataset, batch_size=args.batch, sampler=test_subsampler, drop_last=True)

                net = MySPDNet(args.classes, args.units, args.latents).to(device)
                ...
                Y_preds.append(h_ys_te)
                Y_true.append(ys_te)
                

            Y_preds = np.concatenate(Y_preds)
            Y_true = np.concatenate(Y_true)
            acc = sum((np.argmax(Y_preds, axis=1) == Y_true)) / Y_true.shape[0]
            print(f'Test accuracy: {acc}')
    ...
    sio.savemat('/data2/lalalagegrx/EEGemotion/SEED/Results/SPDNet/SPDNet_1layer_loto_dim.mat', {'dim':dim, 'acc':acc_total})
