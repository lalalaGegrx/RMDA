...
for i=1:subjects
    ...
    for j=1:length(dim_dr)
        [acc_gamma(i,j),Label_gamma(:,j,i),W]=LeaveOneTrialOutCV_PSD(dim_dr(j),PSD_gamma,Label_valance,order,iter);
        %[acc_gamma(i,j),Label_gamma(:,j,i),W]=LeaveOneTrialOutCV_PSD(dim_dr(j),PSD_gamma,Label_arousal,order,iter);
    end
end
